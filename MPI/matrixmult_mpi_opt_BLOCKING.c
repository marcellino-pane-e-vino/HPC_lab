...
int main(int argc, char **argv) {
    ...
    // 5. SUMMA logic fully overlapped with double-buffering
    double *A_buf[2] = {local_A, malloc(local_n_rows*local_n_cols*sizeof(double))};
    double *B_buf[2] = {local_B, malloc(local_n_rows*local_n_cols*sizeof(double))};
    ...
    for (int kk_outer = 0; kk_outer < p_cols; kk_outer++) {
        ...
        // Start non-blocking broadcast into next buffer
        MPI_Ibcast(A_buf[next], local_n_rows*local_n_cols, MPI_DOUBLE,
                   kk_outer % p_cols, row_comm, &reqs[0]);
        MPI_Ibcast(B_buf[next], local_n_rows*local_n_cols, MPI_DOUBLE,
                   kk_outer % p_rows, col_comm, &reqs[1]);

        // If not first iteration, multiply with current buffer while next broadcast is ongoing
        if (kk_outer > 0) {
            // multiply
            for (int ii = 0; ii < local_n_rows; ii += TILE_SIZE) {
                int i_max = MIN(ii + TILE_SIZE, local_n_rows);
                for (int kk = 0; kk < local_n_cols; kk += TILE_SIZE) {
                    int k_max = MIN(kk + TILE_SIZE, local_n_cols);
                    for (int jj = 0; jj < local_n_cols; jj += TILE_SIZE) {
                        int j_max = MIN(jj + TILE_SIZE, local_n_cols);
                        for (int i = ii; i < i_max; i++)
                            for (int k = kk; k < k_max; k++)
                                for (int j = jj; j < j_max; j++)
                                    local_C[i*local_n_cols + j] +=
                                        A_buf[cur][i*local_n_cols + k] * B_buf[cur][k*local_n_cols + j];
                    }
                }
            }
        }

        // Wait for broadcast to complete before swapping buffers
        MPI_Waitall(2, reqs, stats);
        cur = next;
    }

    // multiply last iteration
    for (int ii = 0; ii < local_n_rows; ii += TILE_SIZE) {
        int i_max = MIN(ii + TILE_SIZE, local_n_rows);
        for (int kk = 0; kk < local_n_cols; kk += TILE_SIZE) {
            int k_max = MIN(kk + TILE_SIZE, local_n_cols);
    ...
    if (rank == 0) {
        printf("SUMMA Tiled fully overlapped - Matrix: %d, Procs: %d (%dx%d), Time: %f s\n",
