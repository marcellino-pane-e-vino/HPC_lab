...
int main(int argc, char **argv) {
    ...
    // 3. SCATTER BLOCKS (block distribution)
    ...
    // 5. SUMMA LOGIC WITH TILING (using ikj order)
    for (int kk_outer = 0; kk_outer < p_cols; kk_outer++) {
        // Broadcast blocks along row
        MPI_Bcast(local_A, local_n_rows*local_n_cols, MPI_DOUBLE, kk_outer % p_cols, row_comm);
        // Broadcast blocks along column
        MPI_Bcast(local_B, local_n_rows*local_n_cols, MPI_DOUBLE, kk_outer % p_rows, col_comm);

        // Tiled local multiplication (ikj order)
                        ...
                        for (int k = kk; k < k_max; k++)
                            for (int j = jj; j < j_max; j++)
                                local_C[i*local_n_cols + j] +=
                                    local_A[i*local_n_cols + k] * local_B[k*local_n_cols + j];
                }
            }
        }
    }
    ...
    if (rank == 0) {
        printf("SUMMA Tiled (ikj) - Matrix: %d, Procs: %d (%dx%d), Time: %f s\n",
