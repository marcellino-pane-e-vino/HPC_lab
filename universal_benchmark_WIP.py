...
class Validator:
    ...
    def validate():
        ...
        # Check for valid benchmark mode
        ...
        if MODE == "threads_vs_size":
            # Ensure SINGLE_PROGRAM file exists
            program_path = Path(SINGLE_PROGRAM["file"])
            if not program_path.exists():
                error(f"Source file not found: {program_path}")
            
            # Validate the thread values are realistic for the given machine
            ...
            # Ensure all programs in PROGRAMS have their source files
            ...
            # Validate compilers are available in the PATH
            missing_compilers = []
            for compiler in COMPILERS:
                if subprocess.call(["which", compiler], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) != 0:
                    warn(f"Compiler '{compiler}' not found in PATH.")
                    missing_compilers.append(compiler)

            # If any required compiler is missing, source the Intel OneAPI environment
            for compiler in missing_compilers:
                info(f"Sourcing Intel OneAPI for {compiler}...")
                # Source Intel OneAPI environment for icc/icx compilers
                subprocess.run(
                    ["bash", "-l", "-c", f"source /opt/intel/oneapi/setvars.sh > /dev/null 2>&1"],
                    check=True
                )
                
                # After sourcing, check if compiler is available now
                if subprocess.call(["which", compiler], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) != 0:
                    error(f"Compiler '{compiler}' not found even after sourcing OneAPI. Please install Intel OneAPI properly.")

            # Validate each flagset configuration
            for flags in FLAGSETS:
                for flag in flags:
                    if flag not in FLAG_ALIASES:
                        error(f"Invalid flag alias: {flag}. Check FLAG_ALIASES dictionary.")

            # Validate that at least one compiler is available
            if not any(subprocess.call(["which", compiler], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) == 0 for compiler in COMPILERS):
                error(f"No valid compilers found in PATH. Ensure gcc, icc, or icx is installed.")

        # Check for all required program files to exist
        required_files = ["matrixmult_opt.c", "omp_matrixmult.c", "matrixmult_library.c"]
    ...
    def compiler_vs_flags():
        ...
        for config in FLAGSETS:
            ...
            for compiler in COMPILERS:
                ...
                binary = Compiler.compile(program, compiler, flags)
                times = [Executor.run(binary, SINGLE_MATRIX_SIZE, N_THREADS) for _ in range(RUNS_PER_TEST)]
                row.append(statistics.mean(times))
            rows.append(row)
        return headers, rows
